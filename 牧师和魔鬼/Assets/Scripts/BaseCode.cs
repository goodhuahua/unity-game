...
namespace Com.Mygame {
	...
	public class MyCharacterController {
		...
		public void moveToPosition(Vector3 destination) {
            //调用`moveableScript`的`setDestination()`方法设置目标位置。
        ...
        //将角色放置在船上。将角色的父对象设置为船的游戏对象，设置`_isOnBoat`为`true`。
        public void getOnBoat(BoatController boatCtrl) {
			coastController = null;
			character.transform.parent = boatCtrl.getGameobj().transform;
			_isOnBoat = true;
		}
        //将角色放置在岸边。将角色的父对象设置为`null`，设置`_isOnBoat`为`false`。
        ...
        //重置角色的状态和位置。调用`moveableScript`的`reset()`方法，
		//将角色放置在初始岸边，并设置位置、添加到岸边的角色列表中。
		public void reset() {
	...
	public class BoatController {
        ...
        //构造函数，创建船只的游戏对象，并设置相关属性和位置。
		//同时添加Moveable和ClickGUI组件。
